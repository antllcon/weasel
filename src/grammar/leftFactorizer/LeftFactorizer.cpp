...
namespace
{
	...
	raw::Alternative FindLongestCommonPrefix(const raw::Alternatives& alts)
	{
		...
		for (size_t i = 0; i < alts.size(); ++i)
		{
			for (size_t j = i + 1; j < alts.size(); ++j)
			{
				...
				while (k < alts[i].size() && k < alts[j].size() &&
				       alts[i][k] == alts[j][k] && IsValidPrefixSymbol(alts[i][k]))
				{
	...
	std::string GenerateUniqueName(const std::string& baseName, const raw::Rules& existingRules, const raw::Rules& newRules)
	{
		...
		auto isTaken = [&](const std::string& name) {
			auto check = [&name](const raw::Rule& r) { return r.name == name; };
			return std::ranges::any_of(existingRules, check) || std::ranges::any_of(newRules, check);
		};
