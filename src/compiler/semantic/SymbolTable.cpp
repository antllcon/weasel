...
bool SymbolTable::Declare(const std::string& name, std::shared_ptr<TypeInfo> type, bool isMutable, uint32_t slot, bool isConst, const Expr* constExpr, bool isConstRef)
{
	if (m_scopes.empty())
	{
		return false;
	...
	current[name] = SymbolInfo{std::move(type), slot, isMutable, isConst, constExpr, isConstRef};
	return true;
}
